    ...
    public function rules(): array
    {
        ...
        if ($isFaculty) {
            $rules['faculty_id'] = [
                ...
                'string',
                'max:255',
                Rule::unique('users', 'faculty_id')->ignore($userId),
                function ($attribute, $value, $fail) {
                    $faculty = \App\Models\Faculty::whereRaw(
                        'LOWER(email) = ?',
        ...
        // Prevent assigning both Faculty and Student roles
        if ($facultyRoleId && $studentRoleId) {
            $rules['role_ids'][] = function ($attribute, $value, $fail) use ($facultyRoleId, $studentRoleId) {
                if (is_array($value)) {
                    ...
                    if (
                        in_array((int) $facultyRoleId, $normalized, true) &&
                        in_array((int) $studentRoleId, $normalized, true)
                    ) {
