...
import { CheckCircle2, ChevronDown, ChevronRight, Clock, Lock, Pencil, Power, Shield } from 'lucide-react';
...
interface ActivityTimelineProps {
    ...
    formatTitle?: (event: ActivityEvent) => string;
}
 ...
 */
function groupRelatedEvents(events: ActivityEvent[]): GroupedEvent[] {
    ...
    let currentGroup: ActivityEvent[] | null = null;
    let lastTimestamp = 0;
    ...
    for (const event of sorted) {
        ...
        const shouldGroup = currentGroup && timeDiff < groupTimeThresholdMs && currentGroup[0].action_type === event.action_type;
...
function formatEventTitle(event: ActivityEvent): string {
            ...
            {event.new_values &&
                ...
                Object.keys(event.new_values)
                    ...
                    .map((key) => {
                        ...
                        return (
                            <div key={key} className="overflow-hidden break-words text-muted-foreground">
...
export default function ActivityTimeline({
    ...
    userId,
    fullHistoryHref,
    title = 'System Activity — read-only',
    emptyTitle = 'System Activity',
    emptyDescription = 'No activity recorded yet',
    formatTitle = formatEventTitle,
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                <div className="relative space-y-4">
                    ...
                    {displayGroups.length > 1 && <div className="absolute top-6 bottom-0 left-1.5 w-0.5 bg-border" />}
                        ...
                        <div key={group.id}>
                            ...
                            <div className="relative flex gap-4">
                                ...
                                <div className="relative z-10 mt-1">
                                    <div className="flex h-4 w-4 items-center justify-center rounded-full border-2 border-background bg-white">
                                ...
                                <div className="flex-1 pb-4">
                                    <div
                                        className={`flex flex-col gap-1 ${group.isGrouped ? 'cursor-pointer rounded p-2 hover:bg-muted' : ''}`}
                                        onClick={() => group.isGrouped && toggleGroup(group.id)}
                                    >
                                        <div className="flex items-center gap-2">
                                            {group.isGrouped && (
                                                <ChevronRight
                                                    className={`h-4 w-4 flex-shrink-0 text-muted-foreground transition-transform ${expandedGroups.has(group.id) ? 'rotate-90' : ''}`}
                                                />
                                            )}
                                            <div className="min-w-0 flex-1">
                                                <p className="text-xs leading-tight font-medium md:text-sm">
                                                    {formatTitle(group.events[0])}
                                                    {group.isGrouped && (
                                                        <span className="ml-1 text-xs text-muted-foreground md:ml-2">
                                        ...
                                        <p className="ml-6 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                            {(() => {
                                            ...
                                            <span title={formatAbsolute(group.events[0].created_at)}>
                                                {formatTimestamp(group.events[0].created_at)}
                                            </span>
                                        </p>
                            ...
                            {group.isGrouped && expandedGroups.has(group.id) && (
                                <div className="mb-4 ml-12 space-y-3 border-l-2 border-border pl-4">
                                    {group.events.map((event, idx) => (
                                        <div key={event.id} className="min-w-0 rounded bg-muted/50 p-2">
                                            <p className="mb-1 text-xs font-medium break-words">
                                                {formatTitle(event)}
                                                <span className="ml-2 font-normal text-muted-foreground">{formatTimestamp(event.created_at)}</span>
                                            </p>
                                            <div className="space-y-1 border-l border-border pl-2 text-xs">{renderEventDetails(event)}</div>
                                        </div>
                                    ))}
                                </div>
                            )}
                ...
                {(hasMore || isExpanded) && (
                    <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="w-full text-xs">
