...
interface LogDetailsPayload {
  ...
  report_format_id?: number;
  report_format?: {
    id: number;
    name: string;
  } | null;
  report_format_name?: string | null;
  generated_by?: number;
  ...
  filters_applied?: Record<string, any> | any[] | string | null;
  filters_applied_display?: string | null;
  ...
  const getModalTitle = () => {
    ...
    switch (logType) {
      ...
      case 'generated-reports':
        return 'Generated Reports Log Details';
      default:
        return 'Log Details';
    }
  };
              ...
              {logType === 'generated-reports' && (
                <section className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-base text-gray-900 dark:text-gray-100">
                      {data.report_type_name ?? data.reportType?.name ?? `ID: ${data.report_type_id ?? 'N/A'}`}
                    </p>
                  </div>
                  <div>
                    <p className="text-[11px] md:text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400 font-medium">
                    ...
                    <p className="mt-1 text-base text-gray-900 dark:text-gray-100">
                      {data.report_format_name ?? data.reportFormat?.name ?? `ID: ${data.report_format_id ?? 'N/A'}`}
                    </p>
                  </div>
                  <div>
                    <p className="text-[11px] md:text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400 font-medium">
                    ...
                    <p className="mt-1 text-base text-gray-900 dark:text-gray-100">
                      {data.generated_by_name ?? data.generatedBy?.full_name ?? data.generatedBy?.name ?? `ID: ${data.generated_by ?? 'N/A'}`}
                    </p>
                  </div>
                  <div>
                    <p className="text-[11px] md:text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400 font-medium">
                  ...
                  {(data.filters_applied || data.filters_applied_display) && (
                    <div className="lg:col-span-2">
                      <p className="text-[11px] md:text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400 font-medium">
                        Filters Applied
                      </p>
                      <div className="mt-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 p-3 text-sm text-gray-700 dark:text-gray-200 whitespace-pre-wrap break-words">
                        {typeof data.filters_applied === 'string'
                          ? data.filters_applied
                          : data.filters_applied_display ?? JSON.stringify(data.filters_applied, null, 2)}
                      </div>
                    </div>
                  )}
                </section>
